...
{   
    ...
    public class FAClientSimulator : SocketClientSimulator
    {
        ...
        public FAClientSimulator(int port)
            : base(port, -1, "\r", ' ')
        {
        ...
        protected override void ProcessUnsplitMessage(string message)
        {
            ...
            {
                case "MOV":
                    string reply = message.Replace("MOV", "ACK");
                    SendMessage(reply);
                    break;

                case "GET":
                    if(message.Contains("RECIPELIST"))
                    {
                        SendMessage("ACK:RECIPELIST/Recipe1|Recipe2|Recipe3;");
                        break;
                    }
                    if(message.Contains("RECIPEBODY"))
                    {
                        string reply1 = message.Replace("GET", "ACK").Replace(";", "/ABCDEFG;");
                        SendMessage(reply1);
                        break;
                    }
                    break;
        ...
        internal void SendEvent(string cmd)
        {
