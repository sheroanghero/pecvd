...
namespace MECF.Framework.Simulator.Core.LoadPorts.Hirata
{
    ...
    public class JelRobotSimulator : SerialPortDeviceSimulator
    {
        ...
        protected override void ProcessUnsplitMessage(byte[] data)
        {
            ...
            _msgBuffer.AddRange(data);
            var rawMessage = Encoding.ASCII.GetString(data);
            if (!rawMessage.Contains("\r"))
            {
                return;
            }
            rawMessage = Encoding.ASCII.GetString(_msgBuffer.ToArray());
            _msgBuffer.Clear();

            string message = rawMessage.Replace("$1", "").Replace("\r", "");
            ...
            if(message.Contains("6M"))
            {
                if (message == "6M1")
                {
                    OnWriteMessage($">$1100.111\r");
                }
                if (message == "6M2")
                    OnWriteMessage($">$1+150.222\r");
                if (message == "6M3")
                    OnWriteMessage($">$1+150.333\r");
                if (message == "6M4")
                    OnWriteMessage($">$1+60.444\r");
                return;
            }
            ...
            if (message.Contains("G") && message != "G")     //Home
            {
