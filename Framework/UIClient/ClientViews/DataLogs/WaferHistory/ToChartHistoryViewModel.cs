...
namespace MECF.Framework.UI.Client.CenterViews.DataLogs.WaferHistory
{
    public class ToChartHistoryViewModel : UiViewModelBase
    {
        ...
        public ToChartHistoryViewModel()
        {
            DisplayName = "To Chart History";
        ...
        public void SelectData(object parameter)
        {
            ...
            if (waferHistoryRecipe != null)
            {
                Application.Current.Dispatcher.BeginInvoke(new Action(() =>
                {
                    ...
                    if (waferHistoryRecipe.StartTime != DateTime.MinValue)
                        dtMin = waferHistoryRecipe.StartTime;
                    if (waferHistoryRecipe.EndTime != DateTime.MinValue)
                        dtMax = waferHistoryRecipe.EndTime;
                    else
                        dtMax = waferHistoryRecipe.StartTime.AddHours(1);

                    string chamberType = (string)QueryDataClient.Instance.Service.GetConfig($"System.SetUp.{waferHistoryRecipe.Chamber}.ChamberType");

                    if (chamberType == "PVD")
                    {
                        PMParameters = new ObservableCollection<string>(new[] { $"{waferHistoryRecipe.Chamber}.ChamberPressure",
                            $"{waferHistoryRecipe.Chamber}.DCPower.ForwardPower",$"{waferHistoryRecipe.Chamber}.Mfc1.Feedback",
                            $"{waferHistoryRecipe.Chamber}.Mfc2.Feedback",$"{waferHistoryRecipe.Chamber}.Pressure2.Value",
                            $"{waferHistoryRecipe.Chamber}.ChamberHeater.FeedBack",$"{waferHistoryRecipe.Chamber}.WaterFlow.WaterFlow",
                            $"{waferHistoryRecipe.Chamber}.WaterFlow.Temperature", });

                        QueryData(waferHistoryRecipe, PMParameters, ref valueMin, ref valueMax);
                    }
                    else if (chamberType == "Clean")
                    {
                        PMParameters = new ObservableCollection<string>(new[] { $"{waferHistoryRecipe.Chamber}.ChamberPressure",
                            $"{waferHistoryRecipe.Chamber}.HighPower.ForwardPower", $"{waferHistoryRecipe.Chamber}.HighPower.ReflectPower",
                            $"{waferHistoryRecipe.Chamber}.LowPower.ForwardPower", $"{waferHistoryRecipe.Chamber}.LowPower.ReflectPower",
                            $"{waferHistoryRecipe.Chamber}.Mfc1.Feedback", $"{waferHistoryRecipe.Chamber}.Mfc2.Feedback",
                            $"{waferHistoryRecipe.Chamber}.Pressure2.Value", });

                        QueryData(waferHistoryRecipe, PMParameters, ref valueMin, ref valueMax);
                    }
                    else if (chamberType == "Degas")
                    {
                        PMParameters = new ObservableCollection<string>(new[] { $"{waferHistoryRecipe.Chamber}.Temperature.Temperature",
                            $"{waferHistoryRecipe.Chamber}.PowerRegulator.FeedBack", });

                        QueryData(waferHistoryRecipe, PMParameters, ref valueMin, ref valueMax);
                    }
                    else if (chamberType == null)
                    {
                        string[] charmber = waferHistoryRecipe.Name.Split('.');
                        waferHistoryRecipe.Chamber = charmber[0];
                        PMParameters = new ObservableCollection<string>(new[] { $"{waferHistoryRecipe.ItemInfo}",});
        ...
        public void QueryData(WaferHistoryRecipe recipe, ObservableCollection<string> keys, ref double min, ref double max)
        {
            if (string.IsNullOrEmpty(recipe.StartTime.ToString()))
            {
            ...
            if (!string.IsNullOrEmpty(recipe.EndTime.ToString()))
            {
