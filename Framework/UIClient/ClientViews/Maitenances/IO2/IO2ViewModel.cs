    ...
    public class IO2ViewModel : UiViewModelBase, ISupportMultipleSystem
    {
        ...
        private void ApplyFilter()
        {
            ...
            else
            {
                foreach (var item in DIs)
                {
                    item.IsMatch = item.DisplayName.IndexOf(CurrentCriteria, StringComparison.OrdinalIgnoreCase) >= 0 || item.Name.IndexOf(CurrentCriteria, StringComparison.OrdinalIgnoreCase) >= 0;
                }
