...
namespace MECF.Framework.Common.FAServices
{
    ...
    /// 所有ID 的命名规则：
    ///
    /// Module(1-399) + Type(1-80)  + Unit(1-99) + Parameter (1-999)
    ///
    /// 最小 1 01 01 001  = 10101001
    ///
    /// 最大 399 99 99 999 = 3 999 999 999
    ///
    ...
    public class VIDGenerator
    {
        ...
        public Dictionary<string, Dictionary<string, Dictionary<string, Dictionary<string, int>>>> moduleTypeUnitParamIndex = new Dictionary<string, Dictionary<string, Dictionary<string, Dictionary<string, int>>>>();
        ...
        public void Initialize()
        {
            ...
            try
            {
                ...
                {
                    moduleTypeUnitParamIndex[item.ToString()] = new Dictionary<string, Dictionary<string, Dictionary<string, int>>>();
                });
                ...
                foreach (var VIDItem in VIDList)
                {
                    ...
                    if (!moduleTypeUnitParamIndex.ContainsKey(VIDItem.Module))
                    {
                        moduleTypeUnitParamIndex[VIDItem.Module] = new Dictionary<string, Dictionary<string, Dictionary<string, int>>>();
                    }

                    if (!moduleTypeUnitParamIndex[VIDItem.Module].ContainsKey(VIDItem.Type))
                    {
                        moduleTypeUnitParamIndex[VIDItem.Module][VIDItem.Type] = new Dictionary<string, Dictionary<string, int>>();
                    }

                    if (!moduleTypeUnitParamIndex[VIDItem.Module][VIDItem.Type].ContainsKey(VIDItem.Unit))
                    {
                        moduleTypeUnitParamIndex[VIDItem.Module][VIDItem.Type][VIDItem.Unit] = new Dictionary<string, int>();
                    }

                    moduleTypeUnitParamIndex[VIDItem.Module][VIDItem.Type][VIDItem.Unit][VIDItem.Parameter] = VIDItem.Index;
        ...
        private void AssignNewId(List<VIDItem> dataList)
        {
            ...
            foreach (var data in dataList)
            {
                ...
                if (!moduleTypeUnitParamIndex.ContainsKey(data.Module))
                {
                    moduleTypeUnitParamIndex[data.Module] = new Dictionary<string, Dictionary<string, Dictionary<string, int>>>();
                    moduleTypeUnitParamIndex[data.Module][data.Type] = new Dictionary<string, Dictionary<string, int>>();
                    moduleTypeUnitParamIndex[data.Module][data.Type][data.Unit] = new Dictionary<string, int>();

                    data.ModuleIndex = moduleTypeUnitParamIndex.Keys.ToList().FindIndex(x => x == data.Module) + 1;
                    data.TypeIndex = 1;
                    data.UnitIndex = 1;
                    data.ParameterIndex = 1;

                    moduleTypeUnitParamIndex[data.Module][data.Type][data.Unit][data.Parameter] = int.Parse($"{ data.ModuleIndex}{data.TypeIndex.ToString().PadLeft(2, '0')}{data.UnitIndex.ToString().PadLeft(2, '0')}{ data.ParameterIndex.ToString().PadLeft(3, '0')}");
                    data.Index = moduleTypeUnitParamIndex[data.Module][data.Type][data.Unit][data.Parameter];
                    continue;
                }

                if (!moduleTypeUnitParamIndex[data.Module].ContainsKey(data.Type))
                {
                    moduleTypeUnitParamIndex[data.Module][data.Type] = new Dictionary<string, Dictionary<string, int>>();
                    moduleTypeUnitParamIndex[data.Module][data.Type][data.Unit] = new Dictionary<string, int>();

                    data.ModuleIndex = moduleTypeUnitParamIndex.Keys.ToList().FindIndex(x => x == data.Module) + 1;
                    data.TypeIndex = moduleTypeUnitParamIndex[data.Module].Count;
                    data.UnitIndex = 1;
                    data.ParameterIndex = 1;
                ...
                if (!moduleTypeUnitParamIndex[data.Module][data.Type].ContainsKey(data.Unit))
                {
                    moduleTypeUnitParamIndex[data.Module][data.Type][data.Unit] = new Dictionary<string, int>();

                    data.ModuleIndex = moduleTypeUnitParamIndex.Keys.ToList().FindIndex(x => x == data.Module) + 1;
                    data.TypeIndex = moduleTypeUnitParamIndex[data.Module].Keys.ToList().FindIndex(x => x == data.Type) + 2;
                    data.UnitIndex = moduleTypeUnitParamIndex[data.Module][data.Type].Count;
                    data.ParameterIndex = 1;
                ...
                moduleTypeUnitParamIndex[data.Module][data.Type][data.Unit][data.Parameter] = moduleTypeUnitParamIndex[data.Module][data.Type][data.Unit].Last().Value + 1;
                data.Index = moduleTypeUnitParamIndex[data.Module][data.Type][data.Unit][data.Parameter];
    ...
    public class VIDManager : Singleton<VIDManager>
    {
        ...
        public void Initialize(string equipName, bool enableGem300Events = false, bool needReGenerateGemModelXml = false)
        {
            ...
            //SVID
            var svid = new VIDGenerator("SVID", $"{PathManager.GetCfgDir().Replace("\\bin\\Debug", "")}VIDs\\_SVID.xml");
            svid.Initialize();
            svid.GenerateId(Singleton<DataManager>.Instance.VidDataList);
            ExportSvid(svid.VIDList, true, true);

            //ECID
            var ecid = new VIDGenerator("ECID", $"{PathManager.GetCfgDir().Replace("\\bin\\Debug", "")}VIDs\\_ECID.xml");
            ecid.Initialize();
