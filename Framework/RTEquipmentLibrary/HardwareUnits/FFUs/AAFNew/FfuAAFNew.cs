...
{
    public class FfuAAFNew : BaseDevice, IConnection, IDevice
    {
        private FfuAAFNewConnection _connection;
        ...
        public FfuAAFNew(string scRoot, string module, string name) : base(module, name, name, name)
        {
        ...
        {
            _lstHandler.AddLast(new FfuAAFNewSetGroupAddressHandler(this, DeviceAddress, GroupAddress, new byte[] { 0x00, newGroupAddress }));
        }

        public void GetGroupAddress()
        {
            _lstHandler.AddLast(new FfuAAFNewGetGroupAddressHandler(this, DeviceAddress, GroupAddress, new byte[] { 0x00 }));
        }

        public void SetDeviceAddress(byte newDeviceAddress)
        {
            _lstHandler.AddLast(new FfuAAFNewSetAddressHandler(this, DeviceAddress, GroupAddress, new byte[] { 0x01, newDeviceAddress }));
        }

        public void GetDeviceAddress()
        {
            _lstHandler.AddLast(new FfuAAFNewGetAddressHandler(this, DeviceAddress, GroupAddress, new byte[] { 0x01 }));
        }

        public void QueryMaxSpeed1()
        {
            _lstHandler.AddLast(new FfuAAFNewQueryMaxSpeed1Handler(this, DeviceAddress, GroupAddress, new byte[] { 0x08 }));
        }

        public void QueryMaxSpeed2()
        {
            _lstHandler.AddLast(new FfuAAFNewQueryMaxSpeed2Handler(this, DeviceAddress, GroupAddress, new byte[] { 0x09 }));
        }

        public void QueryMaxSpeed3()
        {
            _lstHandler.AddLast(new FfuAAFNewQueryMaxSpeed3Handler(this, DeviceAddress, GroupAddress, new byte[] { 0x0a }));
        }
        /// <summary>
        /// 查转速因子
        /// </summary>
        ...
        {
            _lstHandler.AddLast(new FfuAAFNewQuerySpeedFactorHandler(this, DeviceAddress, GroupAddress, new byte[] { 0x11 }));
        }

        public void SetSpeedFactor(byte newSpeedFactor)
        {
            _lstHandler.AddLast(new FfuAAFNewSetSpeedFactorHandler(this, DeviceAddress, GroupAddress, new byte[] { 0x11, newSpeedFactor }));
        }

        public void QuerySpeed()
        {
            _lstHandler.AddLast(new FfuAAFNewQuerySpeedHandler(this, DeviceAddress, GroupAddress));
        }

        public void SetSpeed(int speed)
        {
            _lstHandler.AddLast(new FfuAAFNewSetSpeedHandler(this, DeviceAddress, GroupAddress, new byte[] { (byte)(speed * 250.0 / MaxSpeed2) }));
        }

        public void ResetDevice()
        {
            _lstHandler.AddLast(new FfuAAFNewResetDeviceHandler(this, DeviceAddress, GroupAddress));
        }

        public void QueryError()
        {

            _lstHandler.AddLast(new FfuAAFNewQueryStatusHandler(this, DeviceAddress, GroupAddress));
        ...
        public bool Initialize()
        {
            ...
            GroupAddress = (byte)SC.GetValue<int>($"{_scRoot}.{Name}.GroupAddress");
            DATA.Subscribe($"{Name}.GroupAddress", () => GroupAddress);

            Status = SC.GetValue<int>($"{_scRoot}.{Name}.Status");
            DATA.Subscribe($"{Name}.Status", () => StatusStringList[ChangedStatus(Status)]);

            _deviceAddress = address.ToString("D3");
            

            Speedfactor = (byte)SC.GetValue<int>($"{_scRoot}.{Name}.Speedfactor");
            DATA.Subscribe($"{Name}.Speedfactor", () => Speedfactor);


            MaxSpeed1 = SC.GetValue<int>($"{_scRoot}.{Name}.MaxSpeed1");
            DATA.Subscribe($"{Name}.MaxSpeed1", () => MaxSpeed1);

            MaxSpeed2 = SC.GetValue<int>($"{_scRoot}.{Name}.MaxSpeed2");
            DATA.Subscribe($"{Name}.MaxSpeed2", () => MaxSpeed2);

            MaxSpeed3 = SC.GetValue<int>($"{_scRoot}.{Name}.MaxSpeed3");
            DATA.Subscribe($"{Name}.MaxSpeed3", () => MaxSpeed3);

            _connection = new FfuAAFNewConnection(portName);
            ...
            OP.Subscribe($"{Name}.SetFFUSpeed", (cmd, param) =>
            {
                if (param == null)
                {                    
                    int speed = SC.GetValue<int>($"{_scRoot}.{Name}.FanSpeedSetPoint");
                    SetSpeed(speed);
