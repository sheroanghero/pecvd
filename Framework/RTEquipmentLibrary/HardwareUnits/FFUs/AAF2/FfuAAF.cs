    ...
    public class FfuAAF : BaseDevice, IConnection, IDevice
    {
        ...
        private bool _enableLog
        {
            get
            {
                ...
                if (SC.ContainsItem($"{_scRoot}.{Module}.{Name}.EnableLogMessage"))
                    return SC.GetValue<bool>($"{_scRoot}.{Module}.{Name}.EnableLogMessage");
                else if (SC.ContainsItem($"{Module}.{Name}.EnableLogMessage"))
                    return SC.GetValue<bool>($"{Module}.{Name}.EnableLogMessage");
                return false;
            }
        }
        ...
        public FfuAAF(string scRoot, string module, string name) : base(module, name, name, name)
        {
        ...
        public bool Initialize()
        {
            ...
            if(string.IsNullOrEmpty(_scRoot))
            {
                ...
                int address = SC.GetValue<int>($"{Module}.{Name}.DeviceAddress");

                DeviceAddress = (byte)address;
                DATA.Subscribe($"{Name}.DeviceAddress", () => DeviceAddress);

                GroupAddress = (byte)SC.GetValue<int>($"{Module}.{Name}.GroupAddress");
                DATA.Subscribe($"{Name}.GroupAddress", () => GroupAddress);

                Status = SC.GetValue<int>($"{Module}.{Name}.Status");
                DATA.Subscribe($"{Name}.Status", () => StatusStringList[ChangedStatus(Status)]);

                _deviceAddress = address.ToString("D3");

                Speedfactor = (byte)SC.GetValue<int>($"{Module}.{Name}.Speedfactor");

                // _nMaxSpeed = SC.GetValue<int>($"{_scRoot}.{Module}.{Name}.MaxSpeed1");
                MaxSpeed1 = SC.GetValue<int>($"{Module}.{Name}.MaxSpeed1");

                MaxSpeed2 = SC.GetValue<int>($"{Module}.{Name}.MaxSpeed2");
                MaxSpeed3 = SC.GetValue<int>($"{Module}.{Name}.MaxSpeed3");
                _ffuSpeed = SC.GetValue<int>($"{Module}.{Name}.Speed");            }
            else
            {
            ...
            _connection = new FfuAAFConnection(Address);
            ...
            OP.Subscribe($"{Name}SetFFUSpeed", (cmd, param) =>
            {
                if (param == null)
                {
                    string tempName = "FFU1";
                    if (Name == "FfuAAF1")
                    {
                        tempName = "FFU1";
                    }
                    else if(Name == "FfuAAF2")
                    {
                        tempName = "FFU2";
                    }
                    int speed = SC.GetValue<int>($"System.{tempName}FanSpeedSetPoint");
                    SetSpeed(speed);
