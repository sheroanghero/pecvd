...
namespace MECF.Framework.RT.EquipmentLibrary.HardwareUnits.ThrottleValves.VAT
{
    public class VATS650 : VATS651
    {
        ...
        public override VATS651Connection Connection
        {
        ...
        public override AITThrottleValveData DeviceData
        {
            get
            {
                return new AITThrottleValveData()
                {
                    ...
                    UnitPosition = "",
                    UnitPressure = "mTorr",
                    MaxValuePosition = _scPositionCommunicationRange.IntValue,
                    MaxValuePressure = _scPressureSensorRange.IntValue,
        ...
        public bool IsPosMode { get; private set; }
        public bool IsNotOpenOrClose { get; private set; }

        public bool IsOpen => PositionFeedback >= 999;
        public bool IsClose => PositionFeedback <= 0;
        ...
        public VATS650(string module, string name, string scRoot="") : base(module, name, scRoot)
        {
            _scRoot = scRoot;
            base.Module = module;
            base.Name = name;
        ...
        public override bool Initialize()
        {
            ...
            _lstMonitorHandler.AddLast(new VATS651QueryOpenStatusHandler(this));
            _lstMonitorHandler.AddLast(new VATS651QueryModeHandler(this));
            _lstMonitorHandler.AddLast(new VATS651QueryPositionHandler(this));
            _lstMonitorHandler.AddLast(new VATS651QueryPressureHandler(this));
            ...
            OP.Subscribe($"{Module}.{Name}.SetMode", (out string reason, int time, object[] param) =>
            {
                ...
                else if (param[0].ToString().ToLower().Contains("tvopen"))
                {
                    SetOpen();
                }
                else if (param[0].ToString().ToLower().Contains("tvclose"))
                {
                    SetClose();
                }
                else
        ...
        private bool OnTimer()
        {
            try
            {
                ...
                {
                    _trigForceAction.CLK = FuncForceAction(!IsClose);
        ...
        public override void SetPosition(float position)
        {
            ...
            else if (Math.Abs(position) >= 1000)
            {
                SetOpen();
            }
            else
        ...
        internal override void NoteActionCompleted(string command)
        {
            if (command == "C:")
            {
        ...
        internal override void NoteQueryResult(string command, string queryResult)
        {
            if (command == "A:")
            {
                var pos = queryResult.Substring(0, 6);
                float.TryParse(pos, out float posValue);
                Position = queryResult; //0-1000
                SetPositionFeedback(posValue);
            }
            else if (command == "P:")
            {//0-9999
                var pressure = queryResult.Substring(0, 6);
                float.TryParse(pressure, out float pressueValue);
                Pressure = queryResult;
                SetPressureFeedback(pressueValue / 10f);
            }
            ...
            {
                IsPosMode = queryResult == "POS" ? true : false;
                if (IsNotOpenOrClose == true)
                    Mode = IsPosMode ? PressureCtrlMode.TVPositionCtrl : PressureCtrlMode.TVPressureCtrl;
                
            }
            else if (command == "i:05")
            {
                Mode = queryResult == "O" ? PressureCtrlMode.TVOpen : (queryResult == "C" ? PressureCtrlMode.TVClose : PressureCtrlMode.Undefined);
                if (queryResult == "O")
                {
                    State = 1;
                    Mode = PressureCtrlMode.TVOpen;
                    IsNotOpenOrClose = false;
                }
                else if (queryResult == "C")
                {
                    State = 0;
                ...
                {
                    State = 1;
                    IsNotOpenOrClose = true;
                    Mode = IsPosMode ? PressureCtrlMode.TVPositionCtrl : PressureCtrlMode.TVPressureCtrl;
                }
            }
        }
        public override void QueryAllStatus()
        {
            lock (_locker)
            {
        ...
        public override void NoteError(string errorCode)
        {
            if (errorCode != null)
            {
