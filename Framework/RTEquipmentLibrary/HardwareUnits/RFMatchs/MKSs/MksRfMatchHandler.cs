...
namespace MECF.Framework.RT.EquipmentLibrary.HardwareUnits.RFMatchs.MKSs
{
    public abstract class MksRfMatchHandler : HandlerBase
    {
        public MksRfMatch Device { get; }

        private byte _address;
        private byte _command;

        protected MksRfMatchHandler(MksRfMatch device, byte address, byte command, byte[] data)
        ...
        {
            MksRfMatchMessage response = msg as MksRfMatchMessage;
            ResponseMessage = msg;
        ...
        protected virtual void ParseData(MksRfMatchMessage msg)
        {
            if (msg.Data[0] != 0)
            {
    ...
    public class MksRfMatchSetActivePresetHandler : MksRfMatchHandler
    {
        public MksRfMatchSetActivePresetHandler(MksRfMatch device, byte address, byte network, int number)
    ...
    //92 set   preset 
    public class MksRfMatchSetPresetHandler : MksRfMatchHandler
    {
        public MksRfMatchSetPresetHandler(MksRfMatch device, byte address, byte network, Presets data)
    ...
    //93 control mode
    public class MksRfMatchSetControlModeHandler : MksRfMatchHandler
    {
        public MksRfMatchSetControlModeHandler(MksRfMatch device, byte address, byte network, EnumRfMatchTuneMode mode)
    ...
    //94 enable presets
    public class MksRfMatchEnablePresetHandler : MksRfMatchHandler
    {
        public MksRfMatchEnablePresetHandler(MksRfMatch device, byte address, byte network, bool enable)
    ...
    public class MksRfMatchEnableCapMoveHandler : MksRfMatchHandler
    {
        public MksRfMatchEnableCapMoveHandler(MksRfMatch device, byte address, byte network, bool enable)
    ...
    public class MksRfMatchSetLoadPositionHandler : MksRfMatchHandler
    {
        public MksRfMatchSetLoadPositionHandler(MksRfMatch device, byte address, byte network, float position)
    ...
    public class MksRfMatchSetTunePositionHandler : MksRfMatchHandler
    {
        public MksRfMatchSetTunePositionHandler(MksRfMatch device, byte address, byte network, float position)
    ...
    //161 query preset no
    public class MksRfMatchQueryPresetNumberHandler : MksRfMatchHandler
    {
        public MksRfMatchQueryPresetNumberHandler(MksRfMatch device, byte address)
            : base(device, address, 161, null)
        {
            Name = "query preset mode";
        }
        protected override void ParseData(MksRfMatchMessage response)
        {
    ...
    //219 query status
    public class MksRfMatchQueryStatusHandler : MksRfMatchHandler
    {
        private MksStatusData _statusData;

        public MksRfMatchQueryStatusHandler(MksRfMatch device, byte address)
        ...
        protected override void ParseData(MksRfMatchMessage response)
        {
            ...
            else
            {
                ...
                AnalysisMksMatchStatus(retData);

                Device.NoteStatus(_statusData);
            }
        }

        private void AnalysisMksMatchStatus(byte[] sts)
        {
