...
{
    public class EdwardsPump2205 : PumpBase, IConnection
    {
        public float MotorTemperature { get; set; }
        public float MotorElectricity { get; set; }
        ...
        private EdwardsPump2205Connection _connection;
        public EdwardsPump2205Connection Connection
        {
            get { return _connection; }
        }

        protected SCConfigItem _scStopVelocity;
        protected SCConfigItem _scStartVelocity;
        ...
        public override bool IsOn 
        {
            get
            {
                if (Speed > Convert.ToSingle(_scStartVelocity.Value))
                {
                    return true;
                }
                else
        ...
        public bool IsSlow
        {
            get
            {
                if (Speed > Convert.ToSingle(_scStopVelocity.Value) && Speed < Convert.ToSingle(_scStartVelocity.Value))
                {
                    return true;
                }
                else
        ...
        public override AITPumpData DeviceData
        {
            get
            {
                ...
                data.AttrValue.Add("MotorTemperature", MotorTemperature);
                data.AttrValue.Add("MotorElectricity", MotorElectricity);
                data.AttrValue.Add("OperationMode", OperationMode);
                data.AttrValue.Add("ErrorCount", ErrorCount);
                return data;
            }
        }
        ...
        public EdwardsPump2205(string module, string name, string scRoot) : base(module, name)
        {
            _scRoot = scRoot;
        }
        ...
        public override bool Initialize()
        {
            ...
            _portName = SC.GetStringValue($"{Module}.{Name}.DeviceAddress");
            _enableLog = SC.GetValue<bool>($"{(!string.IsNullOrEmpty(_scRoot) ? _scRoot : "{Module}.{Name}")}.EnableLogMessage");
            _connection = new EdwardsPump2205Connection(_portName);
            _connection.EnableLog(_enableLog);
            //_connection.IsEnableHandlerRetry = true;

            _scStopVelocity = SC.GetConfigItem($"System.TurboPump.StopVelocity");
            _scStartVelocity = SC.GetConfigItem($"System.TurboPump.StartVelocity");
            ...
            OP.Subscribe($"{Module}.{Name}.PumpReset", (function, args) =>
            {
                SetPumpReset();
                return true;
            });
            ...
            lock (_locker)
            {
                //_lstHandler.AddLast(new EdwardsPump2205ReadMeasValueHandler(this,null));
                //_lstHandler.AddLast(new EdwardsPump2205ReadMeasHandler(this, null));
            }
            _lstMonitorHandler.AddLast(new EdwardsPump2205ReadMeasValueHandler(this));
            _lstMonitorHandler.AddLast(new EdwardsPump2205ReadModFonctHandler(this));
            return true;
        }
        ...
        public override void SetPumpOnOff(bool isOn)
        {
            lock (_locker)
            {
                ...
                _lstHandler.Clear();
                if (isOn)
                    _lstHandler.AddLast(new EdwardsPump2205RawCommandHandler(this,0x31));
                else
                    _lstHandler.AddLast(new EdwardsPump2205RawCommandHandler(this,0x32));
            }
        }

        private void SetPumpReset()
        {
            _lstHandler.AddLast(new EdwardsPump2205RawCommandHandler(this, 0x34));
        }
