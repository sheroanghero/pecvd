...
namespace MECF.Framework.RT.Core.IoProviders.Omron
{
    ...
    /// <br />
    /// <note type="warning">如果在测试的时候报错误码64，是因为PLC中产生了报警，如伺服报警，模块错误等产生的，但是数据还是能正常读到的，屏蔽64报警或清除plc错误可解决</note>
    /// 地址支持的列表如下：
    /// <list type="table">
    ///   <listheader>
    ///     <term>地址名称</term>
    ...
    public class OmronFinsNet : NetworkDeviceBase<FinsMessage, ReverseWordTransform>
    {
        ...
        public OmronFinsNet(string ipAddress, int port, string _localIp)
        {
            WordLength = 1;
            IpAddress = ipAddress;
            Port = port;
            SA1 = Convert.ToByte(_localIp.Substring(_localIp.LastIndexOf(".") + 1));
            ByteTransform.DataFormat = DataFormat.CDAB;
        }
